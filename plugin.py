...
class BasePlugin:
    ...
    def get_real_time_data(self):
        ...
        params = {'sn': self.inverter_sn, 'variables': []}
        data = self.api_request('post', path, params)

        if data and 'data' in data:
            # Przyjmując, że pole zawiera bieżącą moc
            return data['data'].get('current_power', 0)  # Dostosuj nazwę pola, jeśli konieczne
        return None
    ...
    def get_total_energy(self):
        ...
        if data and 'data' in data:
            # Przyjmując, że pole zawiera całkowitą energię
            return data['data'].get('total_energy', 0)  # Dostosuj nazwę pola, jeśli konieczne
        return None
